<script>
  ...
  async function fetchData() {
    try {
      ...
      const response = await SecurityFetch("/record/data?mode=false", "GET");
  ...
  async function delete_row(OrderNum) {
    ...
    try {
      const response = await SecurityFetch("/delete-row", "DELETE", data);
...
{#if $nickname != null && $nickname != "admin"}
